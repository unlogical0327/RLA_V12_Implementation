...
function [reflect_pool_xy,reflect_pool_dist,reflect_pool_angle,reflect_pool_ID,bad_detected_ID,bad_detected_reflector_polar,dist_err,angle_err,pool_status,queue_status,match_status]=generate_reflector_pool(pool_size,Reflector_map,Reflector_ID,Lidar_expect_xy,pose_expect,detected_pool_polar,detected_pool_ID,detected_dist_max,dist_err_trace,angle_err_trace,thres_near,thres_far,thres_dist,thres_angle)
         ...
         %   disp('Match detected reflectors with ref reflectors....')
            ...
            for l=1:l_det   % detected pool
                for ii=1:len_t                    % ref pool
                   ...
                   %[min_dist(l),idx_dist] = min(delta_dist(:,l));
                   %[min_angle(l),idx_angle] = min(delta_angle(:,l));
                   [min_weight(l),idx_weight] = min(weight(:,l));          
              %if min_dist(l)<thres_dist && min_angle(l)<thres_angle  && idx_dist==idx_angle
              %if delta_dist(ii,l)<thres_dist && delta_angle(ii,l)<thres_angle  
